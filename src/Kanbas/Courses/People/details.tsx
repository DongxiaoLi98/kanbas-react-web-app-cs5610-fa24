...
return (
  <div className="wd-people-details position-fixed top-0 end-0 bottom-0 bg-white p-4 shadow w-25">
    ...
    <div className="text-danger fs-4 wd-name">
      ...
      {/* 1. Function to edit name */}
      {/* If not editing, display the name */}
      {!editing && (
        <div className="wd-name" onClick={() => setEditing(true)}>
          {user.firstName} {user.lastName}
        </div>)
      }
       {/* If editing, display the input field to change */}
      {user && editing && (
        <input 
      ...
      {/* 2. Function to edit email */}
      {/* If not editing, display the email */}
      {!editing && (
      ...
      {/* 3. Function to edit role */}
      {/* If not editing, display the role */}
      {!editing && (
        <div className="wd-role" onClick={() => setEditing(true)}>
          {user.role}
        </div>)
      }
       {/* If editing, display the dropdown to change */}
      {user && editing && (
        <select 
          className="form-control w-50 wd-edit-name"
